...
class LogicalSquareFSM:
    ...
    def generate_class_code(self):
        ...
        initial_state_id = min(initial_states) if initial_states else None
    ...
    # generuje kod klas z nazwami:  class State1cName(State)
    ...
    #         """
    #         Usuwa znaki specjalne i spacje z nazwy stanu oraz zamienia pierwszą literę na wielką.
    #         """
    ...
    #         # Sprawdź, czy stan jest nierozwinięty (nie ma dzieci)
    #         if not node["children"]:
    #             state = node.get("state")  # Pobierz stan, jeśli istnieje
    #             state_name_suffix = sanitize_name(state.name) if state and getattr(state, 'name', None) else ""
    #             state_class_name = f"State{state_id}{state_name_suffix}"  # Zbuduj nazwę klasy
    #
    #             class_code += f"class {state_class_name}(State):\n"
    #             class_code += f"    def handle_event(self, context):\n"
    #
    #             # Logika asercji (jeśli istnieje)
    #             if state:
    #                 assertion = state.assert_state()
    #                 assertion_code = self.generate_assertion_code(assertion)
    #                 if assertion_code:
    #                     class_code += f"        # Assertion logic\n"
    #                     class_code += f"        print('{state_class_name}: handling event {assertion}')\n"
    #             class_code += "\n"
    #
    #             # Logika przejść dla tego stanu
    #             transitions_for_state = self.state_transitions_map.get(state_id, {})
    ...
    #                     class_code += f"            context.set_state(State{to_state}())\n"
    #                     class_code += "            return\n"
    #                 class_code += "        print('No valid transition for event')\n"
    #             else:
    #                 class_code += f"        print('No transitions defined for {state_class_name}')\n"
    #             class_code += "\n\n"
    #
    #     # 3. Generowanie klasy kontekstowej StateMachineContext
    #     class_code += "class StateMachineContext:\n"
    ...
    #
    #     if initial_state_id is not None:
    #         # Pobieranie nazwy stanu początkowego (jeśli istnieje)
    #         state = self.span_tree[initial_state_id].get("state")
    #         state_name_suffix = sanitize_name(state.name) if state and getattr(state, 'name', None) else ""
    #         # Konstrukcja nazwy klasy
    #         initial_state_class_name = f"State{initial_state_id}{state_name_suffix}"
    #
    #         class_code += "\n        # Initial state\n"
    #         class_code += f"        self.current_state = {initial_state_class_name}()\n"
    #     else:
    #         class_code += "\n        # Initial state\n"
    #         class_code += "        self.current_state = None\n"
    #         class_code += "        print('Error: No initial state available')\n"
    #
    #     # Metoda set_state
    #     class_code += "\n    def set_state(self, state):\n"
